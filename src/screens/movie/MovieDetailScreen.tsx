...
export default function MovieDetailScreen() {
  ...
  const days = useMemo(() => {
    ...
    }).filter((day, index, allDays) => allDays.findIndex((candidate) => candidate.key === day.key) === index).slice(0, 4);
  }, [showtimes]);
  ...
  const dayShowtimes = selectedDay
    ? showtimes.filter((showtime) => new Date(showtime.starts_at).toDateString() === selectedDay)
    : showtimes;
  const selectedShowtime = dayShowtimes[0];
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.topBar}>
        ...
        <Text style={styles.logo}>Movie details</Text>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.metaLine}>
          ...
          <Text style={styles.metaText}>{movie.genre.toUpperCase()}</Text>
        ...
        <View style={styles.actionLine}>
          ...
          <Pressable
            onPress={() => Alert.alert('Trailer unavailable', 'A trailer link has not been added for this movie yet.')}
            style={styles.trailerAction}
          >
        ...
        <Text style={styles.sectionTitle}>Choose a showtime</Text>
        <View style={styles.venueRow}>
          ...
          <Pressable onPress={() => router.push('/(protected)/(tabs)/cinemas')}>
        ...
        <View style={styles.days}>
          {days.map((day, index) => (
            <Pressable
              ...
              style={[styles.dayPill, (selectedDay === day.key || (!selectedDay && index === 0)) && styles.activeDayPill]}
            >
              <Text style={[styles.dayMonth, (selectedDay === day.key || (!selectedDay && index === 0)) && styles.activeDayText]}>{day.month}</Text>
              <Text style={[styles.dayNumber, (selectedDay === day.key || (!selectedDay && index === 0)) && styles.activeDayText]}>{day.day}</Text>
              <Text style={[styles.dayWeek, (selectedDay === day.key || (!selectedDay && index === 0)) && styles.activeDayText]}>{index === 0 ? 'TODAY' : day.weekday}</Text>
            </Pressable>
          ))}
        </View>

        <View style={styles.times}>
          {dayShowtimes.slice(0, 4).map((showtime, index) => (
      ...
      <View style={styles.reserveBar}>
        <Pressable accessibilityLabel="Save movie" onPress={() => setSaved((current) => !current)} style={styles.bookmarkButton}>
          <Bookmark color={colors.gold} fill={saved ? colors.gold : 'transparent'} size={22} />
        </Pressable>
        {selectedShowtime ? (
...
const styles = StyleSheet.create({
  ...
  logo: { color: colors.text, fontSize: 16, fontWeight: '900' },
  ...
  posterHero: { height: 300, overflow: 'hidden' },
  heroShade: { ...StyleSheet.absoluteFill, backgroundColor: 'rgba(0,0,0,0.28)' },
  ...
  title: { color: colors.text, fontSize: 31, lineHeight: 35, fontWeight: '900', paddingHorizontal: 16, marginTop: 7 },
  ...
  trailerAction: { flexDirection: 'row', alignItems: 'center', gap: 7 },
  sectionKicker: { color: colors.gold, fontSize: 10, marginTop: 20, marginBottom: 9, paddingHorizontal: 16 },
  sectionTitle: { color: colors.text, fontSize: 23, fontWeight: '900', marginTop: 24, marginBottom: 12, paddingHorizontal: 16 },
  ...
  dayPill: { flex: 1, minHeight: 64, borderRadius: 12, borderColor: colors.border, borderWidth: 1, alignItems: 'center', justifyContent: 'center', gap: 1, backgroundColor: colors.panel },
  ...
  timeCard: { width: '48%', minHeight: 58, borderRadius: 12, borderColor: colors.borderBright, borderWidth: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.panel },
  ...
  reserveBar: { position: 'absolute', left: 0, right: 0, bottom: 0, padding: 12, paddingBottom: 15, flexDirection: 'row', gap: 10, backgroundColor: 'rgba(11,11,12,0.96)', borderTopColor: colors.border, borderTopWidth: 1 },
  bookmarkButton: { width: 48, height: 48, borderRadius: 14, borderColor: colors.borderBright, borderWidth: 1, alignItems: 'center', justifyContent: 'center' },
  reserveButton: { flex: 1, height: 48, borderRadius: 14, backgroundColor: colors.gold, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 },
